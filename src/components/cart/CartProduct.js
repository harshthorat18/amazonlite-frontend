...
const CartProduct = (props) => {
  ...
  async function deleteFromCart() {
    try {
      const res = await axios.delete(`${API_BASE_URL}api/delete/${product.id}`, {
    ...
    <div className='cart-product'>
      <div className='product-left'>
        <div className='product-img-wrapper'>
          <img className='product-img' src={product.url} alt='img' />
        </div>
        <div className='product-details'>
          ...
          <img src='/images/amazon-fulfilled.png' alt='fulfilled' />
          <div className='product-options' id="product-options">
            <section className='quantity'>Qty: {qty}</section>
            <div className='delete' onClick={deleteFromCart}>Delete</div>
